...
test('transformSvg: single stroke path gets class, pathLength, style', () => {
  ...
  assert.match(output, /<style>[\s\S]*\.draw-line[\s\S]*@keyframes phosphor-draw-in[\s\S]*<\/style>/);
...
test('transformSvg: style block uses mount-play animation with configured duration and easing', () => {
  const input = `<svg viewBox="0 0 256 256"><path stroke="red" d="M0,0"/></svg>`;
  const { output } = transformSvg(input, { duration: 2, stagger: 0.5, easing: 'linear' });
  assert.match(output, /\.draw-line\s*\{[^}]*animation: phosphor-draw-in 2s linear forwards/);
  assert.match(output, /@keyframes phosphor-draw-in/);
  assert.match(output, /to\s*\{ stroke-dashoffset: 0; \}/);
  assert.doesNotMatch(output, /svg:hover/);
});
